...
Berry = function(options, target) {
	 ...
	 * @param {element} target Target is the admin element where new fields should be added
	...
	// Give renderers and other plugins a chance to admin this
	...
	Berry.normalizeItem = function(item, i, defaultItem){
		...
		// Set a sensible type admin if type is not defined or not found
...
Berry.processOpts = function(item, object) {
	...
	if(typeof item.choices !== 'undefined' && item.choices.length > 0) {
		...
		// Insert the admin value at the beginning
		// if(typeof item.admin !== 'undefined') {
		// 	item.choices.unshift(item.admin);
		// }
	...
	// else{
	// 			// Insert the admin value at the beginning
	// 	if(typeof item.admin !== 'undefined' && typeof item.options !== 'undefined') {
	// 		item.options.unshift(item.admin);
	// 	}
	// }
