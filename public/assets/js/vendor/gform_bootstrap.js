var gform = function (optionsIn, el) {
    ...
    //initalize form
    // this.options = _.assignIn({fields:[], legend: '',strict:true, admin:gform.admin||{}, data:'search', columns:gform.columns,name: gform.getUID()},this.opts, data);
    // this.options2 = _.assign({fields:[], legend: '',strict:true, admin:gform.admin, data:'search', columns:gform.columns,name: gform.getUID()},this.opts, data);
...
//     }, this.options.admin,(gform.types[fieldIn.type]||gform.types['text']).defaults, fieldIn)
    ...
    normalize: function (form) {
        return (options, field) => {
            ...
            if (!('type' in field) || !(field.type in gform.types)) {
                ...
                console.warn('Field type "' + old + '" not supported - using ' + field.type + " " + ((field.columns == 0) ? "(columns set to 0)" : (('options' in field) ? '(admin collection)' : '(admin)')) + ' instead');
            }
